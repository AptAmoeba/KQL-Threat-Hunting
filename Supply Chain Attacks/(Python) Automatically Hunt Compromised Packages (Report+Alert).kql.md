...
let LinuxSysInstallLibPath = @"/usr/(local/)?(lib|lib64)/python[^/]+/(site|dist)-packages/[^/]+";//ssssystem-wide installs (dnf/apt + sudo pip), both site-packages (RHEL/Rocky/ALMA/ALS) and dist-packages (Debian/Uwuntu)
let LinuxUserInstallLibPath = @"/home/[^/]+/\.local/lib/python[^/]+/site-packages/[^/]+";//pip userland installs 
let LinuxOptInstallLibPath = @"/opt/[^/]+/lib/python[^/]+/site-packages/[^/]+";//custom apps in /opt
let CompromisedLibs =
...
| where FolderPath matches regex UserInstallLibPath
      ...
      or FolderPath matches regex LinuxSysInstallLibPath
      or FolderPath matches regex LinuxUserInstallLibPath
      or FolderPath matches regex LinuxOptInstallLibPath
| extend LibraryFolder = tolower(coalesce(
    extract(@"site-packages\\([^\\]+)", 1, FolderPath),
    extract(@"(?:site|dist)-packages/([^/]+)", 1, FolderPath)))
